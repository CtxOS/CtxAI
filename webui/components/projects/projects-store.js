...
const model = {
  ...
  getSelectedProjectSkillsPath() {
    ...
    return `usr/projects/${projectName}/.ctxaiproj/skills/`;
  },
  ...
  async browseInstructionFiles() {
    await this.browseSelected(".ctxaiproj", "instructions");
    try {
  ...
  async browseKnowledgeFiles() {
    await this.browseSelected(".ctxaiproj", "knowledge");
  ...
  getSelectedAbsPath(...relPath) {
    return ["/ctxai/usr/projects", this.selectedProject.name, ...relPath]
      .join("/")
      .replace(/\/+/g, "/");
  },
