(define(
      ...
      (r.inherits(a, s),
        function () {
          function e(e, t) {
            var n = e.exec(t);
            if (n) {
              var r = n[0].includes("set") ? "set" : n[1].toLowerCase();
              if (r) {
                var i = n[0].toLowerCase().indexOf(r);
                return n.index + i + 1;
              }
            }
          }
          function t(e, t) {
            var n,
              r = e.$tokenIndex,
              i = t ? "punctuation.begin" : "punctuation.end";
            e.step = t ? e.stepBackward : e.stepForward;
            while ((n = e.step())) {
              if (n.type !== i) continue;
              break;
            }
            if (!n) return;
            var s = e.getCurrentTokenPosition();
            return (
              t || (s.column = s.column + n.value.length),
              (e.$tokenIndex = r),
              s
            );
          }
          ((this.getFoldWidgetRangeBase = this.getFoldWidgetRange),
            ...
            (this.foldingStartMarkerNunjucks =
              /(?:\{%-?\s*)(?:(block|if|else|elif|for|asyncEach|asyncAll|macro|filter|call)\b.*)|(?:\bset(?:[^=]*))(?=%})/i),
            (this.foldingStopMarkerNunjucks =
              /(?:\{%-?\s*)(endblock|endif|endfor|endeach|endall|endmacro|endfilter|endcall|endset)\b.*(?=%})/i),
            (this.getFoldWidgetRange = function (t, n, r) {
              var i = t.doc.getLine(r),
                s = e(this.foldingStartMarkerNunjucks, i);
              return s
                ? this.nunjucksBlock(t, r, s)
                : ((s = e(this.foldingStopMarkerNunjucks, i)),
                  s
                    ? this.nunjucksBlock(t, r, s)
                    : this.getFoldWidgetRangeBase(t, n, r));
            }),
            (this.getFoldWidget = function (t, n, r) {
              var i = t.getLine(r),
                s = this.foldingStartMarkerNunjucks.test(i),
                o = this.foldingStopMarkerNunjucks.test(i);
              if (s && !o) {
                var u = e(this.foldingStartMarkerNunjucks, i);
                if (u) {
                  var a = t.getTokenAt(r, u).type;
                  if (a === "keyword.control") return "start";
                }
              }
              if (o && !s && n === "markbeginend") {
                var u = e(this.foldingStopMarkerNunjucks, i);
                if (u) {
                  var a = t.getTokenAt(r, u).type;
                  if (a === "keyword.control") return "end";
                }
              }
              return this.getFoldWidgetBase(t, n, r);
            }),
            (this.nunjucksBlock = function (e, n, r) {
              var i = new u(e, n, r),
                s = i.getCurrentToken();
              if (!s || s.type != "keyword.control") return;
              var a = s.value,
                f = [a],
                l = this.indentKeywords[a];
              if (a === "else" || a === "elif") l = 1;
              if (!l) return;
              var c = t(i, l === -1);
              if (!s) return;
              i.step = l === -1 ? i.stepBackward : i.stepForward;
              while ((s = i.step())) {
                if (s.type !== "keyword.control") continue;
                var h = l * this.indentKeywords[s.value];
                if (s.value === "set") {
                  var p = i.getCurrentTokenPosition(),
                    d = e.getLine(p.row).substring(p.column);
                  if (!/^[^=]*%}/.test(d)) continue;
                }
                if (h > 0) f.unshift(s.value);
                else if (h <= 0) {
                  f.shift();
                  if (!f.length) break;
                  h === 0 && f.unshift(s.value);
                }
              }
              if (!s) return null;
              var v = t(i, l === 1);
              return l === 1 ? o.fromPoints(c, v) : o.fromPoints(v, c);
            }));
        }.call(a.prototype));
    },
  ),
        ...
        u = function (e) {
          ...
          this.$rules = {
            no_regex: [
              ...
              { token: "keyword", regex: "(?:" + r + ")\\b", next: "start" },
  ...
  define(
    ...
    function (e, t, n) {
      "use strict";
      var r = e("../lib/oop"),
        i = e("./folding/nunjucks").FoldMode,
        s = e("../lib/lang"),
        o = e("./html").Mode,
        u = e("./nunjucks_highlight_rules").NunjucksHighlightRules,
        a = [
          "area",
          "base",
          "br",
        ...
        l = function () {
          ((this.HighlightRules = u),
            (this.foldingRules = new i(this.voidElements, s.arrayToMap(f))));
        };
      (r.inherits(l, o),
        function () {
          ((this.$id = "ace/mode/nunjucks"),
            (this.voidElements = s.arrayToMap(a)));
        }.call(l.prototype),
        (t.Mode = l));
    },
  ));
