...
const model = {
  ...
  async loadDevices() {
    ...
    this.selectedDevice =
      this.devices.length > 0 ? this.devices[0].deviceId : "";
  },
  ...
  async pollForDevices() {
    ...
    if (
      this.devices.some((d) => d.deviceId && d.deviceId !== "") ||
      this.permissionAttempts >= 60
    ) {
      this.requestingPermission = false;
      return;
    }
  ...
  getSelectedDevice() {
    ...
    if (!device && this.devices.length > 0) {
      device =
        this.devices.find((d) => d.deviceId === "default") || this.devices[0];
    }
    return device;
  },
};
