...
const model = {
  ...
  get hasUnsavedChanges() {
    return (
      this._toComparableJson(this.settings) !==
  ...
  get scopeMismatchMessage() {
    ...
    if (selectedProject === loadedProject && selectedProfile === loadedProfile)
      return "";
  ...
  async loadConfigList() {
    ...
    try {
      ...
      if (!result.ok)
        this.configsError = result.error || "Failed to load configurations";
    } catch (e) {
  ...
  async resetToDefault() {
    ...
    const confirmed = await showConfirmDialog({
      ...
      message:
        "This will replace the current settings with the plugin defaults. Any unsaved changes will be lost.",
      confirmText: "Reset",
      type: "warning",
    });
    if (!confirmed) return;
