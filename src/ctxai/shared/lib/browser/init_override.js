...
//     if (out.name.startsWith("data-") && out.name != "data-ctxaiUID" && out.name != "data-ctxai-frame-id") return null;
...
//   window.__ctxai_extractFrameContent = function() {
...
//           node.setAttribute("data-ctxaiUID", uid);
//         }
...
//         if (tagName) {
//           content += ` selector="${node.getAttribute("data-ctxaiUID")}"`;
//         }
...
//     if (event.data === 'ctxai_REQUEST_CONTENT') {
//       // Extract content and send it back to parent
//       const content = window.__ctxai_extractFrameContent();
//       // Use '*' as targetOrigin since we're in a controlled environment
//       window.parent.postMessage({
//         type: 'ctxai_FRAME_CONTENT',
//         content: content,
//         frameId: window.frameElement?.getAttribute('data-ctxai-frame-id')
//       }, '*');
//     }
//   });

//   // Function to extract content from all frames
//   window.__ctxai_extractAllFramesContent = async function(rootNode = document) {
//     let content = "";
    
//     // Extract content from current document
//     content += window.__ctxai_extractFrameContent();
...
//         iframe.setAttribute('data-ctxai-frame-id', frameId);
        
//         // Setup one-time message listener for this specific frame
//         const listener = function(event) {
//           if (event.data?.type === 'ctxai_FRAME_CONTENT' && 
...
//         iframe.contentWindow.postMessage('ctxai_REQUEST_CONTENT', '*');
...
//       const frameId = iframe.getAttribute('data-ctxai-frame-id');
