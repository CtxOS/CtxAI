...
from ctxai.utils.plugins import (
    META_FILE_NAME,
    PluginMetadata,
        ...
        try:
            with zipfile.ZipFile(zip_path, "r") as z:
                ...
                for member in z.namelist():
                    ...
                    if (
                        not member_path.startswith(real_extract + os.sep)
                        and member_path != real_extract
                    ):
...
def install_from_git(url: str, token: str | None = None) -> dict:
    """Clone git repo into usr/plugins/, validate plugin.yaml.
    Returns dict with plugin name and metadata."""
    from ctxai.integrations.git.git import clone_repo
