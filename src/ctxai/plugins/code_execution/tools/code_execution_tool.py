...
from ctxai.shared.tool import Tool, Response
from ctxai.shared import files, rfc_exchange, projects, runtime, settings
from ctxai.shared.print_style import PrintStyle
from ctxai.shared.strings import truncate_text as truncate_text_string
from ctxai.shared.messages import truncate_text as truncate_text_agent
from ctxai.shared import plugins
from core.tools.security import ToolScope
...
class CodeExecution(Tool):
    ...
    async def before_execution(self, **kwargs):
        ...
        if runtime_arg == "python":
            self.required_scopes = [ToolScope.PYTHON_EXEC]
        elif runtime_arg == "terminal" or runtime_arg == "nodejs":
            self.required_scopes = [ToolScope.BASH_EXEC]
    ...
    async def prepare_state(self, cfg: dict, reset=False, session: int | None = None):
        ...
        if session is not None and session not in shells:
            ...
            env_id = f"{self.agent.context.id}_{session}"
            
            if ssh_enabled:
                # Still using specialized SSH provider for now to minimize regression
                ...
                # Use SandboxManager for local executions
                shell = await SandboxManager.get_environment(
                    env_id, 
                    provider="local", 
                    cwd=cwd,
                    user_id=self.agent.context.user_id,
                    workspace_id=self.agent.context.workspace_id
                )
    ...
    async def ensure_cwd(self) -> str | None:
        ...
        normalized = files.normalize_ctxai_path(path)
