...
from ctxai.shared.git import extract_author_repo
from ctxai.shared import yaml as yaml_helper
from ctxai.shared.plugins import (
    META_FILE_NAME,
    PluginMetadata,
    get_plugins_list,
    clear_plugin_cache,
...
def install_uploaded_zip(plugin_file: FileStorage) -> dict:
    ...
    tmp_dir = Path(files.get_abs_path(files.TMP_DIR, "plugin_uploads"))
...
def install_from_zip(zip_path: str, original_filename: str | None = None) -> dict:
    ...
    base_tmp = files.get_abs_path(files.TMP_DIR, "plugin_installs")
...
def install_from_git(url: str, token: Optional[str] = None) -> dict:
    ...
    Returns dict with plugin name and metadata."""
    from ctxai.shared.git import clone_repo
...
def fetch_plugin_index() -> dict:
    ...
    index_url = "https://github.com/ctxos/ctxai-plugins/releases/download/generated-index/index.json"
