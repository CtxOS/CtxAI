...
from ctxai.shared import guids

# from langchain_chroma import Chroma
from langchain_community.vectorstores import FAISS

# faiss needs to be patched for python 3.12 on arm #TODO remove once not needed
from ctxai.shared import faiss_monkey_patch
import faiss
...
from ctxai.shared import files, plugins, projects
from langchain_core.documents import Document
from . import knowledge_import
from ctxai.core.common.log import Log, LogItem
...
class Memory:
    ...
    def initialize(
        ...
        em_dir = files.get_abs_path(
            files.TMP_DIR, "memory/embeddings"
...
def get_custom_knowledge_subdir_abs(agent: Agent) -> str:
    for dir in agent.config.knowledge_subdirs:
        if dir != "default":
            if dir == "custom":
                return files.get_abs_path(files.USER_DIR, "knowledge")
            return files.get_abs_path(files.USER_DIR, "knowledge", dir)
...
def abs_db_dir(memory_subdir: str) -> str:
    ...
    if memory_subdir.startswith("projects/"):
        from ctxai.shared.projects import get_project_meta

        return files.get_abs_path(get_project_meta(memory_subdir[9:]), "memory")
    # standard subdirs
    return files.get_abs_path(files.USER_DIR, "memory", memory_subdir)
...
def abs_knowledge_dir(knowledge_subdir: str, *sub_dirs: str) -> str:
    ...
    if knowledge_subdir == "default":
        return files.get_abs_path(files.USER_DIR, "knowledge", *sub_dirs)
    if knowledge_subdir == "custom":
        return files.get_abs_path(files.USER_DIR, "knowledge", *sub_dirs)
    return files.get_abs_path(files.USER_DIR, "knowledge", knowledge_subdir, *sub_dirs)
...
def get_existing_memory_subdirs() -> list[str]:
    try:
        from ctxai.shared.projects import (
            get_project_meta,
            get_projects_parent_folder,
        )

        # Get subdirectories from memory folder
        subdirs = files.get_subdirectories(files.get_abs_path(files.USER_DIR, "memory"))
