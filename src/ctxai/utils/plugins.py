...
from ctxai.utils import cache, files, print_style
from ctxai.utils import yaml as yaml_helper
...
def get_plugin_roots(plugin_name: str = "") -> list[str]:
    ...
    return [
        ...
        files.get_abs_path(files.BUILTIN_DIR, files.PLUGINS_DIR, plugin_name),
    ]
...
def get_enhanced_plugins_list(custom: bool = True, builtin: bool = True) -> list[PluginListItem]:
    ...
    if builtin:
        load_plugins(files.get_abs_path(files.BUILTIN_DIR, files.PLUGINS_DIR), False)
    return results
...
def find_plugin_dir(plugin_name: str):
    ...
    default_plugin_path = files.get_abs_path(
        files.BUILTIN_DIR, files.PLUGINS_DIR, plugin_name, META_FILE_NAME
    )
    if files.exists(default_plugin_path):
        return files.get_abs_path(files.BUILTIN_DIR, files.PLUGINS_DIR, plugin_name)
...
def get_plugin_config(
    ...
    if project_name is None and agent is not None:
        from ctxai.utils import projects
...
def find_plugin_assets(
    *subpaths: str,
    ...
    from ctxai.utils import projects, subagents
    ...
    # plugins/<plugin_name>/...
    path = files.get_abs_path(files.BUILTIN_DIR, files.PLUGINS_DIR, plugin_name, *subpaths)
    _collect(path, "", "")
