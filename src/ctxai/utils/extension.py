...
from ctxai.utils import cache, extract_tools, files, subagents

if TYPE_CHECKING:
    from ctxai.core.agent import Agent
...
def get_webui_extensions(
    ...
    # 1. Search in enabled plugins
    for plugin_name in plugins.get_enabled_plugins(agent):
        plugin_dir = plugins.find_plugin_dir(plugin_name)
        if not plugin_dir:
            continue

        folder = files.get_abs_path(plugin_dir, "extensions/webui", extension_point)
        if not files.exists(folder):
            continue

        for filter_pattern in effective_filters:
            pattern = files.get_abs_path(folder, filter_pattern)
            for path in files.find_existing_paths_by_pattern(pattern):
                rel_path = files.deabsolute_path(path)
                entries.append({"plugin_id": plugin_name, "path": rel_path})

    # 2. Search in other folders (project, user Agents, etc) handled by subagents.get_paths
    # For now, we only support plugin-based webui extensions with explicit IDs
