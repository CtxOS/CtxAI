...
DEFAULT_AGENTS_DIR = f"{files.BUILTIN_DIR}/{files.AGENTS_DIR}"
USER_AGENTS_DIR = f"{files.USER_DIR}/{files.AGENTS_DIR}"
...
def get_all_agents_list() -> list[dict[str, str]]:
    def _origin_from_root(root: str) -> Origin:
        ...
        if rel.startswith(f"{files.DATA_DIR}/projects/"):
            return "project"
        if rel.startswith(USER_AGENTS_DIR):
            return "user"
