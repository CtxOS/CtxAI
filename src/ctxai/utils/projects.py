...
from ctxai.utils import dirty_json, file_tree, files, persist_chat
...
def load_project_subagents(name: str) -> dict[str, SubAgentSettings]:
    try:
        abs_path = files.get_abs_path(get_project_meta(name), "subagents.json")
...
def load_project_secrets_masked(name: str, merge_with_global=False):
    from ctxai.utils import secrets
...
def save_project_secrets(name: str, secrets: str):
    from ctxai.utils.secrets import get_project_secrets_manager
