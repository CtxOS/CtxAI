...
from ctxai.utils import yaml

# Leaf directory names
AGENTS_DIR = "agents/templates"
PLUGINS_DIR = "plugins"
...
DATA_DIR = "data"
USER_DIR = "data/usr"
BUILTIN_DIR = "src/ctxai"

# Unique root-level locations
LOGS_DIR = "data/logs"
TMP_DIR = "data/tmp"
...
def load_plugin_variables(
    ...
    if plugin_file and exists(plugin_file):
        from ctxai.utils import extract_tools
...
from ctxai.utils.strings import sanitize_string
...
def is_probably_binary_file(
    ...
    try:
        ...
        raise OSError(f"Unable to read file for binary detection: {file_path}") from e
...
def get_base_dir():
    # Get the base directory (project root) from the current file path
    # src/ctxai/utils/files.py -> src/ctxai/utils -> src/ctxai -> src -> root
