...
from ctxai.utils.state_monitor import _ws_debug_enabled
...
class WebSocketManager:
    def __init__(self, socketio: AsyncServer, lock) -> None:
    ...
    async def route_event_all(
        ...
        *,
        ...
        self._debug(
            f"Completed requestAll namespace={namespace} for '{event_type}' "
    ...
    def _buffer_event(
        ...
        while len(buffer) > BUFFER_MAX_SIZE:
            ...
            PrintStyle.warning(
                f"Dropping buffered event '{dropped.event_type}' for "
                f"namespace={namespace} sid={sid} (overflow)"
            )
        self._debug(
            f"Buffered event namespace={namespace} '{event_type}' sid={sid} "
            f"(queue length={len(buffer)})"
        )
