...
from plugins.memory.tools.memory_load import (
    DEFAULT_THRESHOLD as DEFAULT_MEMORY_THRESHOLD,
)

from ctxai.integrations.search.searxng import search as searxng
from ctxai.utils import dotenv, duckduckgo_search, perplexity_search
from ctxai.utils.document_query import DocumentQueryHelper
from ctxai.utils.errors import handle_error
from ctxai.utils.print_style import PrintStyle
from ctxai.utils.tool import Response, Tool
...
class Knowledge(Tool):
    ...
    async def searxng_document_qa(self, result, query):
        ...
        for index, _item in enumerate(result["results"]):
    ...
    async def mem_search_enhanced(self, question: str):
        ...
        try:
            ...
            result_parts.append(
                f"**Found:** {len(knowledge_sources)} knowledge sources, {len(conversation_memories)} conversation memories{threshold_note}"
            )
