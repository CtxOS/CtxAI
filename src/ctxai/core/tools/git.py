...
def is_official_ctxai_repo() -> bool:
    ...
    try:
        ...
        return any(
            remote_url.endswith(f"github.com/{repo_name}") or remote_url.endswith(f"github.com:{repo_name}")
...
# Files to ignore when checking dirty status (ctxai project metadata)
CTXAI_IGNORE_PATTERNS = {".ctxaiproj", ".ctxaiproj/"}
...
def get_repo_status(repo_path: str) -> dict:
    ...
    try:
        ...
        try:
            current_branch = (
                repo.active_branch.name if not repo.head.is_detached else f"HEAD@{repo.head.commit.hexsha[:7]}"
            )
        except Exception:
            current_branch = "unknown"

        # Check dirty status, excluding ctxai metadata
        def is_ctxai_file(path: str) -> bool:
            return path.startswith(".ctxaiproj") or path == ".ctxaiproj"

        # Filter out ctxai files from diff and untracked
        changed_files = [d.a_path for d in repo.index.diff(None)] + [d.a_path for d in repo.index.diff("HEAD")]
        untracked = repo.untracked_files

        real_changes = [f for f in changed_files if not is_ctxai_file(f)]
        real_untracked = [f for f in untracked if not is_ctxai_file(f)]
