...
from ctxai.shared.strings import sanitize_string
...
class Tool:
    ...
    async def before_execution(self, **kwargs):
        # Security validation
        from ctxai.core.tools.security import security_registry
        if not security_registry.validate_tool_access(self.name, self.required_scopes):
            raise ToolExecutionError(f"Permission denied for tool '{self.name}'")
