...
from ctxai.shared import files, dirty_json, persist_chat, file_tree
...
PROJECTS_PARENT_DIR = files.PROJECTS_DIR
PROJECT_META_DIR = ".ctxaiproj"
...
def clone_git_project(name: str, git_url: str, git_token: str, data: BasicProjectData):
    """Clone a git repository as a new ctxai project. Token is used only for cloning via http header."""
    from ctxai.shared import git
    ...
    try:
        ...
        # Check if cloned repo already has .ctxaiproj
...
def build_system_prompt_vars(name: str):
    ...
    return {
        ...
        "project_path": files.normalize_ctxai_path(get_project_folder(name)),
...
def load_project_secrets_masked(name: str, merge_with_global=False):
    from ctxai.shared import secrets
...
def save_project_secrets(name: str, secrets: str):
    from ctxai.shared.secrets import get_project_secrets_manager
