...
from ctxai.shared import yaml

AGENTS_DIR = os.path.join("src", "ctxai", "agent", "profiles")
PROMPTS_DIR = os.path.join("src", "ctxai", "agent", "prompts")
TOOLS_DIR = os.path.join("src", "ctxai", "agent", "tools")
SKILLS_DIR = os.path.join("src", "ctxai", "agent", "skills")
PLUGINS_DIR = os.path.join("src", "ctxai", "plugins")
EXT_DIR = os.path.join("src", "ctxai", "ext")
API_DIR = os.path.join("src", "ctxai", "api")

DATA_DIR = "data"
PROJECTS_DIR = os.path.join(DATA_DIR, "projects")
USER_DIR = os.path.join(DATA_DIR, "usr")
LOGS_DIR = os.path.join(DATA_DIR, "logs")
TMP_DIR = os.path.join(DATA_DIR, "tmp")
KNOWLEDGE_DIR = os.path.join("src", "ctxai", "core", "knowledge")
...
def load_plugin_variables(
    ...
    if plugin_file and exists(plugin_file):
        ...
        from ctxai.shared import extract_tools
...
def get_abs_path_dockerized(*relative_paths):
    "Ensures the abs path is dockerized (i.e. /ctxai/... path)"
    abs = get_abs_path(*relative_paths)
    from ctxai.shared import runtime

    if runtime.is_dockerized():
        return abs
    return normalize_ctxai_path(abs)
...
def fix_dev_path(path: str):
    "On dev environment, convert /ctxai/... paths to local absolute paths"
    from ctxai.shared.runtime import is_development

    if is_development():
        if path.startswith("/ctxai/"):
            path = path.replace("/ctxai/", "")
    return get_abs_path(path)


def normalize_ctxai_path(path: str):
    "Convert absolute paths into /ctxai/... paths"
...
def get_base_dir():
    # Filesystem: /.../ctxai/src/ctxai/core/common/files.py
    # 1st dirname -> /.../ctxai/src/ctxai/core/common
    # 2nd dirname -> /.../ctxai/src/ctxai/core
    # 3rd dirname -> /.../ctxai/src/ctxai
    # 4th dirname -> /.../ctxai/src
    # 5th dirname -> /.../ctxai (Project Root)
