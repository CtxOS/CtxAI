...
from ctxai.shared.errors import format_error
from ctxai.shared import files, cache
...
def requires_api_key(f):
    ...
    async def decorated(*args, **kwargs):
        from ctxai.shared.settings import get_settings
...
def register_api_route(app: Flask, lock: ThreadLockType) -> None:
    from ctxai.shared.extract_tools import load_classes_from_file
    ...
    async def _dispatch(path: str) -> BaseResponse:
        ...
        builtin_file = files.get_abs_path(files.API_DIR, f"{path}.py")
        if files.is_in_dir(builtin_file, files.get_abs_path(files.API_DIR)) and files.exists(builtin_file):
