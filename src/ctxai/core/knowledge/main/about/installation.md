...
You can map just the `/ctxai/usr` directory (recommended) or individual subfolders of `/ctxai` to a local directory.

> [!CAUTION]
> Do **not** map the entire `/ctxai` directory: it contains the application code and can break upgrades.
...
```bash
docker run -p 0:80 -v /path/to/your/work_dir:/ctxai/usr ctxos/ctxai
```
...
> [!NOTE]
> Since v0.9.7, custom prompts belong in `/ctxai/agents/<agent_name>/prompts/` rather than a shared `/prompts` folder. See the [Extensions guide](../developer/extensions.md#prompts) for details.
...
| `embedding_llm` | The embedding model shipped with CTXAI runs on CPU and is responsible for generating embeddings used for memory retrieval and knowledge base lookups. Changing the `embedding_llm` will re-index all of CTXAI's memory. |
...
> [!TIP]
> If the new instance fails to load settings, remove `/ctxai/data/usr/settings.json` and restart to regenerate default settings.
...
Ctx AI settings can be automatically configured using environment variables with the `CTXAI_SET_` prefix in your `.env` file. This enables automated deployments without manual configuration.
...
```env
# Model configuration
CTXAI_SET_chat_model_provider=anthropic
CTXAI_SET_chat_model_name=claude-3-5-sonnet-20241022
CTXAI_SET_chat_model_ctx_length=200000

# Memory settings
CTXAI_SET_memory_recall_enabled=true
CTXAI_SET_memory_recall_interval=5

# Agent configuration
CTXAI_SET_agent_profile=custom
CTXAI_SET_agent_memory_subdir=production
```
...
docker run -p 50080:80 \
  -e CTXAI_SET_chat_model_provider=anthropic \
  -e CTXAI_SET_chat_model_name=claude-3-5-sonnet-20241022 \
  ctxos/ctxai
```
...
If you are migrating from older, non-Docker setups, CTXAI handles the migration of legacy folders and files automatically at runtime. The right place to save your files and directories is `ctxai/usr`.
