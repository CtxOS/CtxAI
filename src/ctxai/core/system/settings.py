...
from ctxai.shared import runtime, whisper, defer, git, subagents
from ctxai.core.common import files, dotenv
from ctxai.shared.print_style import PrintStyle
from ctxai.shared.providers import get_providers, FieldOption as ProvidersFO
from ctxai.shared.secrets import get_default_secrets_manager
from ctxai.shared import dirty_json
...
def get_default_value(name: str, value: T) -> T:
    """
    Load setting value from .env with CTXAI_SET_ prefix, falling back to default.

    Args:
        name: Setting name (will be prefixed with CTXAI_SET_)
        value: Default value to use if env var not set

    Returns:
        Environment variable value (type-normalized) or default value
    """
    env_value = dotenv.get_dotenv_value(f"CTXAI_SET_{name}", dotenv.get_dotenv_value(f"CTXAI_SET_{name.upper()}", None))
    ...
    try:
        ...
        PrintStyle(background_color="yellow", font_color="black").print(
            f"Warning: Invalid value for CTXAI_SET_{name}='{env_value}': {e}. Using default: {value}"
        )
        return value
...
class Settings(TypedDict):
    ...
    persistence_redis_host: str
    persistence_redis_port: int
    job_queue_provider: str
    job_queue_redis_host: str
    job_queue_redis_port: int
...
SETTINGS_FILE = files.get_abs_path(files.USER_DIR, "settings.json")
...
def get_default_settings() -> Settings:
    ...
    return Settings(
        ...
        workdir_path=get_default_value("workdir_path", files.get_abs_path_dockerized(files.USER_DIR, "workdir")),
        ...
        # Horizontal Scaling
        persistence_provider=get_default_value("persistence_provider", "in-memory"),
        persistence_redis_host=get_default_value("persistence_redis_host", "localhost"),
        persistence_redis_port=get_default_value("persistence_redis_port", 6379),
        job_queue_provider=get_default_value("job_queue_provider", "local"),
        job_queue_redis_host=get_default_value("job_queue_redis_host", "localhost"),
        job_queue_redis_port=get_default_value("job_queue_redis_port", 6379),
    )
    ...
    if _settings:
        ...
        if not previous or current_token != previous["mcp_server_token"]:
            ...
            async def update_mcp_token(token: str):
                from ctxai.shared.mcp_server import DynamicMcpProxy
        ...
        if not previous or current_token != previous["mcp_server_token"]:
            ...
            async def update_a2a_token(token: str):
                from ctxai.shared.fasta2a_server import DynamicA2AProxy
