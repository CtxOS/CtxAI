...
from ctxai.shared import files, print_style, yaml as yaml_helper, cache
...
def get_plugin_roots(plugin_name: str = "") -> List[str]:
    ...
    return [
        files.get_abs_path(files.USER_DIR, "plugins", plugin_name),
...
def find_plugin_assets(
    *subpaths: str,
    ...
    from ctxai.shared import projects, subagents
    ...
    # project/.ctxaiproj/agents/<profile>/plugins/<plugin_name>/...
    if project_name:
        if agent_profile:
        ...
        if not agent_profile or agent_profile == "*":
            # project/.ctxaiproj/plugins/<plugin_name>/...
            path = projects.get_project_meta(
