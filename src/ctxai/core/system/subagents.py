...
GLOBAL_DIR = "."
USER_DIR = files.USER_DIR
DEFAULT_AGENTS_DIR = files.AGENTS_DIR
USER_AGENTS_DIR = os.path.join(files.USER_DIR, "agents")
PROMPTS_DIR = files.PROMPTS_DIR
...
def get_agents_roots() -> list[str]:
    ...
    project_agents = files.find_existing_paths_by_pattern(os.path.join(files.USER_DIR, "projects", "*", ".ctxaiproj", "agents"))
    paths = [
...
def get_all_agents_list() -> list[dict[str, str]]:
    def _origin_from_root(root: str) -> Origin:
        ...
        if rel.startswith(os.path.join(files.USER_DIR, "projects").replace("\\", "/")):
            return "project"
        if rel.startswith(USER_AGENTS_DIR.replace("\\", "/")):
            return "user"
        if f"/{files.PLUGINS_DIR}/" in rel or rel.startswith(files.PLUGINS_DIR.replace("\\", "/")):
            return "plugin"
        return "default"
...
def get_default_promp_file_names() -> list[str]:
    return files.list_files(PROMPTS_DIR, filter="*.md")
