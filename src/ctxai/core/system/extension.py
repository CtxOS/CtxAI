...
from ctxai.shared import extract_tools, files
from ctxai.shared import cache, plugins, subagents
...
import os
USER_EXTENSIONS_FOLDER = os.path.join(files.USER_DIR, "extensions")
...
def get_webui_extensions(
    ...
    from ctxai.core.system import plugins

    for extension_path in extensions:
        rel_path = files.deabsolute_path(extension_path)
        
        plugin_id = None
        # Check if it belongs to a system plugin
        abs_plugins_dir = files.get_abs_path(files.PLUGINS_DIR)
        if extension_path.startswith(abs_plugins_dir):
            plugin_id = extension_path[len(abs_plugins_dir):].lstrip("/").split("/")[0]
        else:
            # Check if it belongs to a user plugin
            abs_user_plugins_dir = files.get_abs_path(files.USER_DIR, files.PLUGINS_DIR)
            if extension_path.startswith(abs_user_plugins_dir):
                plugin_id = extension_path[len(abs_user_plugins_dir):].lstrip("/").split("/")[0]
