...
UPLOAD_FOLDER = "/ctx/src/ctxai/usr/uploads"
...
def send_message(context: "AgentContext", item: dict, source: str = " (from queue)"):
    ...
    from ctxai.agent import UserMessage  # Import here to avoid circular import

    message = item.get("text", "")
    attachments = item.get("attachments", [])
    log_user_message(context, message, attachments, source=source)
    context.communicate(UserMessage(message=message, attachments=attachments))
...
def send_all_aggregated(context: "AgentContext") -> int:
    ...
    context.communicate(UserMessage(message=text, attachments=attachments))
    return len(items)
