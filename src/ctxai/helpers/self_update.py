...
from ctxai.helpers import git, yaml
...
UPDATE_FILE_PATH = Path("/exe/ctxai-self-update.yaml")
STATUS_FILE_PATH = Path("/exe/ctxai-self-update-status.yaml")
LOG_FILE_PATH = Path("/exe/ctxai-self-update.log")
...
def _get_remote_branch_merged_tags(branch: str) -> set[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="ctxai-self-update-tags-") as temp_dir:
...
def _get_remote_branch_head_info(branch: str) -> dict[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="ctxai-self-update-head-") as temp_dir:
        repository = Path(temp_dir)
        _run_git(repository, "init", "--bare")
        _run_git(
            repository,
...
def get_selector_tag_options(
    ...
    *,
    ...
    current_major = _parse_major_version(current_version or get_repo_version_info(repository)["short_tag"])
...
def schedule_update(
    *,
    ...
    if normalized_tag not in {option["value"] for option in selector_tag_options}:
        raise ValueError(f"Version {normalized_tag} does not exist on branch {normalized_branch}.")
