...
from ctxai.helpers import files, git, runtime
...
class BackupService:
    ...
    def __init__(self):
        ...
        self.agent_zero_root = files.get_abs_path("")  # Resolved Ctx AI root
...
"""
    ...
    async def test_patterns(self, metadata: dict[str, Any], max_files: int = 1000) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise Exception(f"Error processing patterns: {str(e)}") from e
    ...
    async def inspect_backup(self, backup_file) -> dict[str, Any]:
        ...
        try:
            ...
            raise Exception("Invalid backup file: not a valid zip archive") from None
        except json.JSONDecodeError:
            raise Exception("Invalid backup file: corrupted metadata") from None
        finally:
            # Cleanup
    ...
    async def preview_restore(
        ...
        try:
            ...
            raise Exception(f"Error previewing restore: {str(e)}") from e
        finally:
            # Cleanup
            if os.path.exists(temp_file):
                os.remove(temp_file)
    ...
    async def restore_backup(
        ...
        try:
            ...
            raise Exception(f"Error restoring backup: {str(e)}") from e
        finally:
            # Cleanup
            if os.path.exists(temp_file):
                os.remove(temp_file)
