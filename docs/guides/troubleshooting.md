...
- Place the files/dirs in `/ctxai/usr`. Ctx AI will be able to perform tasks on them.
...
- Chat history lives at `/ctxai/usr/chats/` inside the container.
...
Secrets are stored in `/ctxai/usr/secrets.env` and are not always included in backup archives. Copy them manually.
