...
* **Import Knowledge:** Import external files into the agent's knowledge base
  ...
  - Files are stored in `/ctxai/knowledge/custom/main`
...
Access the chat history in JSON format
  ...
  - Files are stored under `/ctxai/usr/chats/` inside the container
...
- **Isolated workspace** under `/ctxai/usr/projects/<project_name>/`
...
> [!NOTE]
> If the cloned repository already contains a `.ctxaiproj/` configuration folder, Ctx AI merges the existing configuration with your specified preferences.
...
- **Variables** (non-sensitive): Stored in `.ctxaiproj/variables.env`
  ```bash
  API_BASE_URL=https://api.example.com
  OUTPUT_FORMAT=json
  ```

- **Secrets** (sensitive): Stored in `.ctxaiproj/secrets.env`
  ```bash
...
The default ignores: `.ctxaiproj/`, `venv/`, `__pycache__/`, `node_modules/`, `.git/`
...
├── .ctxaiproj/                    # Project metadata (managed by A0)
...
> [!IMPORTANT]
> Secrets are stored in `/ctxai/usr/secrets.env`.

> [!NOTE]
> Project-scoped secrets and variables (when using Projects) live under `/ctxai/usr/projects/<project_name>/.ctxaiproj/` (`secrets.env`, `variables.env`).
...
> [!NOTE]
> The file browser lets you navigate the Ctx AI filesystem. For file-based work, keep your working files in `/ctxai/usr` (or inside a Project workspace).
>
- **File Operations**:
...
> [!NOTE]
> Chat history is stored at `/ctxai/usr/chats/` inside the container.
...
> Secrets stored in `/ctxai/usr/secrets.env` are not always included in backup archives. Keep a manual copy if you rely on secrets.
...
#### Security Considerations
* **Secrets**: Backups do **not** reliably include `/ctxai/usr/secrets.env`. Copy it manually when migrating.
