...
**Parameters:**
*   `paths` (array, required): Array of file paths to retrieve (e.g., `["/ctxai/usr/uploads/file.txt"]`)
...
const filePaths = [
    "/ctxai/usr/uploads/document.txt",
    "/ctxai/usr/uploads/data.json"
];
getFiles(filePaths);
...
async function attachmentWorkflow() {
    ...
    if (messageResponse.ok) {
        ...
        const retrievedFiles = await getFiles(["/ctxai/usr/uploads/test.txt"]);
